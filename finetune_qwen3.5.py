...
from utils import get_model, resolve_hf_path
    ...
    if tokenizer.eos_token_id is None:
        for tok in ["<|im_end|>", "<|endoftext|>"]:
            ...
            if isinstance(tok_id, int) and tok_id >= 0 and tok_id != tokenizer.unk_token_id:
                tokenizer.eos_token_id = tok_id
                tokenizer.eos_token = tok
                break

    if tokenizer.eos_token_id is None:
        raise ValueError("Tokenizer does not provide a valid eos token id.")
...
def get_generation_eos_token_ids(tokenizer):
    ...
    if not eos_ids:
        raise ValueError("Cannot resolve eos token ids for generation.")
    if len(eos_ids) == 1:
        return eos_ids[0]
    return eos_ids
...
def main():
    ...
    # get the tokenizer
    tokenizer = get_tokenizer_qwen35(args)
