...
This runner treats each script file as a complete config.
Example file names:
  updated_span_scripts/qwen/sfkl/train_0.6B_4B_sfkl_kd0.7_wrel1.0.sh
  updated_span_scripts/qwen/fdd/train_0.6B_4B_srkl_kd0.5_wrel1.0_fdd0.2.sh
...
Options:
  ...
  --gpus-per-job <n>             GPUs used by each script. Default: 2
...
Examples:
  ./running.sh
  ./running.sh --filter sfkl
  ./running.sh --filter kd0.7_wrel1.0 --gpus 0,1,2,3 --gpus-per-job 2
  ./running.sh --mode sequential --filter csd
